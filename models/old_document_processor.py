...
class DocumentProcessor:
    ...
    def extract_text_multi_method(self, file_path: str) -> Dict[str, Any]:
        ...
        # Method 1: PyMuPDF
        try:
            results['pymupdf'] = self.extract_text_pymupdf(file_path)
            results['pymupdf_confidence'] = 0.9 if len(results['pymupdf']) > 100 else 0.3
        except Exception as e:
            results['pymupdf'] = ""
            results['pymupdf_confidence'] = 0.0

        # Method 2: Azure Form Recognizer
        try:
            results['azure'] = self.extract_text_azure(file_path)
            results['azure_confidence'] = 0.8
        except Exception as e:
            results['azure'] = ""
            results['azure_confidence'] = 0.0

        # Method 3: EasyOCR (fallback)
        if all(len(results[method]) < 50 for method in ['pymupdf', 'azure']):
            try:
                results['easyocr'] = self.extract_text_easyocr(file_path)
                results['easyocr_confidence'] = 0.6
            except Exception as e:
                results['easyocr'] = ""
                results['easyocr_confidence'] = 0.0

        # Select best result
        if results:
            best_method = max(
                [k for k in results.keys() if not k.endswith('_confidence')],
                key=lambda k: len(results[k]) * results.get(f"{k}_confidence", 0)
            )
        else:
            best_method = 'pymupdf'

        return {
            'text': results.get(best_method, ''),
            'method_used': best_method,
            'confidence': results.get(f"{best_method}_confidence", 0),
            'all_results': results
        }
    ...
    def extract_text_easyocr(self, file_path: str) -> str:
        ...
        for page in pages:
            results = self.easyocr_reader.readtext(page)
    ...
    def detect_document_types_on_page(self, page_text: str) -> List[Tuple[str, float]]:
        """Detect multiple document types on a single page - FIXED VERSION"""
        ...
        # I-797 Notice of Action - Enhanced detection
        i797_indicators = [
        ...
        print(f"DEBUG: I-797 indicators: {i797_indicators}")
        ...
        # I-140 Petition
        if 'i-140' in text_lower or 'immigrant petition' in text_lower:
            detections.append(('I140', 0.85))
            print("DEBUG: Detected I-140")
        ...
        if 'birth certificate' in text_lower:
            detections.append(('BIRTH_CERT', 0.9))
        if 'marriage certificate' in text_lower:
            detections.append(('MARRIAGE_CERT', 0.9))
        if any(phrase in text_lower for phrase in ['diploma', 'degree', 'transcript']):
            detections.append(('EDUCATION', 0.7))
    ...
    def process_document_segment(self, segment: DocumentSegment, options: Dict) -> Dict:
        ...
        try:
            if segment.doc_type in ['I797', 'I140']:
                print("DEBUG: Extracting USCIS form data...")
                segment_result['extracted_data'] = self.extract_uscis_form_data(segment.text)
                print(f"DEBUG: USCIS extraction result: {segment_result['extracted_data']}")
    ...
    def extract_uscis_form_data(self, text: str) -> Dict:
        """Extract USCIS form data using LLM"""
        print("DEBUG: Getting USCIS prompt...")
        prompt = get_document_specific_prompt('USCIS')
    ...
    def extract_dol_form_data(self, text: str) -> Dict:
        """Extract DOL form data"""
        prompt = get_document_specific_prompt('DOL')
    ...
    def extract_passport_data(self, text: str) -> Dict:
        ...
        prompt = get_document_specific_prompt('PASSPORT')
        return self.extract_with_llm(text, prompt)

    def extract_visa_data(self, text: str) -> Dict:
        """Extract visa stamp data"""
        prompt = get_document_specific_prompt('VISA')
        return self.extract_with_llm(text, prompt)
    ...
    def check_person_data_consistency(self, person_record: Dict):
        ...
        names = [
            doc['data'].get('beneficiary') or doc['data'].get('full_name')
            for doc in documents
            if doc['data'].get('beneficiary') or doc['data'].get('full_name')
        ]
        unique_names = set(filter(None, names))  # Filter out None values
        if len(unique_names) > 1:
            inconsistencies.append(f"Name variations: {', '.join(unique_names)}")

        # Check DOB consistency
        dobs = [
            doc['data'].get('date_of_birth')
            for doc in documents
            if doc['data'].get('date_of_birth')
        ]
        unique_dobs = set(filter(None, dobs))  # Filter out None values
        if len(unique_dobs) > 1:
            inconsistencies.append(f"DOB variations: {', '.join(map(str, unique_dobs))}")

        # Check citizenship consistency
        countries = [
            doc['data'].get('country_of_citizenship')
            for doc in documents
            if doc['data'].get('country_of_citizenship')
        ]
        unique_countries = set(filter(None, countries))  # Filter out None values
        if len(unique_countries) > 1:
            inconsistencies.append(f"Citizenship variations: {', '.join(unique_countries)}")
    ...
    def process_single_document(self, file_path: str, document_type: str, options: Dict) -> Dict:
        ...
        try:
            ...
            print(f"DEBUG: Attempting to create person record for: '{person_name}'")
